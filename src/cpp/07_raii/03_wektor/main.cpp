...
class Wektor {
public:
    // Konstruktor przyjmujący długość wektora i tworzący wektor wypełniony zerami.
    Wektor(int dlugosc) : dlugosc(dlugosc), pojemnosc(dlugosc), dane(new double[dlugosc]()) {}
    ...
    int pobierzDlugosc() const {
        return dlugosc;
    }

    // Metoda zwracająca pojemność wektora.
    int pobierzPojemnosc() const {
        return pojemnosc;
    }
...
private:
    int dlugosc;
    int pojemnosc;
    double* dane; // Wskaźnik na dynamicznie zaalokowaną tablicę przechowującą elementy wektora.
};
