...
int main() {
  ...
  // stworzenie drugiego wektora tego samego rozmiaru i wypełnienie go sinusami
  // wartości z pierwszego wektora
  std::vector<double> sinValues(values.size());
  std::transform(values.begin(), values.end(), sinValues.begin(),
                 SinusFunctor());
