...
int polaczGniazdo(short uchwytGniazda) {
   ...
   adresSerwera.sin_addr.s_addr = htonl(INADDR_ANY); // Adres IP serwera (zostawiamy domyślny)
   adresSerwera.sin_port = htons(portKlienta); // Port serwera (przekazujemy wartość zdefiniowaną w #define)

   return bind(uchwytGniazda,(struct sockaddr *)&adresSerwera,sizeof(adresSerwera));;
}
...
int main(int argc, char *argv[]) {
    ...
    int wynikProbyPolaczenia = accept(uchwytGniazda,(struct sockaddr *)&klient,(socklen_t*)&dlugoscKlienta);
    ...
    printf("Oczekiwanie na klienta...\n");
    wynikProbyPolaczenia = accept(uchwytGniazda,(struct sockaddr *)&klient,(socklen_t*)&dlugoscKlienta);
    ...
    if( send(wynikProbyPolaczenia, wiadomoscOdebrana, strlen(wiadomoscOdebrana), 0) < 0)  {
